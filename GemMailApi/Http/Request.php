<?php
/**
 * This file contains the GemMailApi_Http_Request class used in the GemMailApi PHP-SDK.
 *
...
/**
 * GemMailApi_Http_Request is the request class used to send the requests to the API endpoints.
 *
 ...
 * @since 1.0
 */
class GemMailApi_Http_Request extends GemMailApi_Base
{
    /**
     * @var GemMailApi_Http_Client the http client injected.
     */
    public $client;

    /**
     * @var GemMailApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     *
     * @param GemMailApi_Http_Client $client
     */
    public function __construct(GemMailApi_Http_Client $client)
    {
        $this->client = $client;
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'GemMailApi Client version '. GemMailApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new GemMailApi_Params($client->paramsPost);
        ...
        $params = $this->params = new GemMailApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new GemMailApi_Params($headers));
        }

        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = GemMailApi_Json::decode($body, true);
        ...
        $params->add('body', new GemMailApi_Params($decodedBody));

        $response = new GemMailApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new GemMailApi_Params($specialHeaderParams);
